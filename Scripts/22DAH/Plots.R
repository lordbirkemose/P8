...
load("./Rdata/22DAH/importanceFeatureSelection.RData")
...
dataWLS <- read.csv("./Data/22DAH/resultsWLS.csv") %>%
  as_tibble() %>%
  mutate(Start = as.Date(Start)) %>%
  mutate(
    Type = factor(
...
dataRF <- read.csv("./Data/22DAH/resultsRF.csv") %>%
  as_tibble() %>%
  mutate(Start = as.Date(Start)) %>%
  mutate(
    Type = factor(
...
dataXGB <- read.csv("./Data/22DAH/resultsXGB.csv") %>%
  as_tibble() %>%
  mutate(Start = as.Date(Start)) %>%
  mutate(
    Type = factor(
...
dataARFIMA <- read.csv("./Data/22DAH/resultsARFIMA.csv") %>%
  as_tibble() %>%
  mutate(Start = as.Date(Start)) %>%
  mutate(
    Type = factor(
