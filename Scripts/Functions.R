...
funAcfPlot <- function(data, lag.max = 50, include.lag.zero = FALSE) {
  ...
  plot <- ggplot2::ggplot(data = acfData, ggplot2::aes(x = lag, y = acf)) +
    ...
    ggplot2::geom_hline(yintercept = confInt, linetype = 2, color = colors[2]) +
    ggplot2::geom_hline(yintercept = -confInt, linetype = 2, color = colors[2]) +
...
funResultsPlots <- function(data, labelsData) {
  ...
  p <- ggplot2::ggplot(data = data) +
    ggplot2::geom_line(ggplot2::aes(x = Start, y = Value, color = Var)) +
    ggplot2::labs(
      # title = "OLS",
      x = "Time",
      y = "Value"
    ...
    ggplot2::facet_wrap(
      Type~ .,
      scales = "free",
      labeller = ggplot2::labeller(Type = labelsData)
    ) +
    ggplot2::annotate(
      geom = "segment",
      y = -Inf, yend = -Inf,
    ...
    ggplot2::geom_text(
      data = textData,
      mapping = ggplot2::aes(x = x, y = -Inf, label = label),
      vjust = -.5,
      size = 3
    ) +
    ggplot2::geom_vline(
